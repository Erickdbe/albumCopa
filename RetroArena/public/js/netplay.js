...
function Netplay(protocols,master) {
	...
	if (netModes.length) {
		...
		function setRoomPlayers(roomplayers) {
			...
			roomplayers.forEach((player,id)=>{
				...
				if (player.isYou) {
					...
					joinedPlayer.label=player.name;
			...
			var menu=[
				{
					...
					default:(window.RETRO_USERNAME||("p"+QMATH.floor(1000+QMATH.random()*8999))).slice(0,12)
				},
		...
		this.show=function(ret) {
			...
			netplay.show(!ret,this,netMode,setupGame.setup);
			netplayTimer=0;
		}
