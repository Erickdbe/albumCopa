function NetplaySocketIo(master) {
	...
	var
		...
		inPlayersEvents,
		connectionSetup;
	const socketEvent=(code)=>"retro-arena:net:"+code;
	...
	this.initialize=function(){
		if (window.io) {
			...
			socket = window.RETRO_SOCKET || io({auth:{token:localStorage.getItem("mp_token")}});
			return true;
		} else return false;
	}
	...
	this.show=function(first,listen,netmode,initialSetup) {		
		listener=listen;
		if (window.RETRO_ROOM_ID) CONFIG.netRoom=window.RETRO_ROOM_ID;
		connectionSetup=initialSetup;
		if (first) this.setState(NETPLAYSTATE_OFF);
		else {
			socket.emit(socketEvent(NETCODE_LEAVEROOM));
	...
	this.manageConnection=function(setup) {
		if (setup&&Object.keys(setup).length) connectionSetup=setup;
		setup=connectionSetup;
		switch (this.state) {
			case NETPLAYSTATE_OFF:{
				socket.off(socketEvent(NETCODE_ROOMCONFIG));
				socket.off(socketEvent(NETCODE_CONFIRM));
				socket.off(socketEvent(NETCODE_ROOMFROZEN));
				socket.off(socketEvent(NETCODE_SETUP));
				socket.off(socketEvent(NETCODE_DATA));
				socket.off(socketEvent(NETCODE_EVENT));
				this.setState(NETPLAYSTATE_PREPARING);
				break;
			}
			case NETPLAYSTATE_PREPARING:{
				socket.on(socketEvent(NETCODE_ROOMCONFIG),(data)=>{
				...
				socket.on(socketEvent(NETCODE_CONFIRM),(data)=>{
					listener.onPlayerConfirm(data);
				});
				socket.on(socketEvent(NETCODE_ROOMFROZEN),(data)=>{
					if (this.state>=NETPLAYSTATE_JOIN) listener.onStartGame(data);
				});
				socket.on(socketEvent(NETCODE_SETUP),(data)=>{
					if (this.state>=NETPLAYSTATE_JOIN) listener.onSetupUpdated(data);
				});
				socket.on(socketEvent(NETCODE_DATA),(data)=>{
					if (this.state==NETPLAYSTATE_PLAYING) {
				...
				socket.on(socketEvent(NETCODE_EVENT),(data)=>{
			...
			case NETPLAYSTATE_JOIN:{
				if (window.RETRO_ROOM_ID) CONFIG.netRoom=window.RETRO_ROOM_ID;
				socket.emit(socketEvent(NETCODE_JOINROOM), [CONFIG.netRoom,CONFIG.netNickname,setup]);
	...
	this.updateSetup=function(setup) {
		socket.emit(socketEvent(NETCODE_UPDATESETUP),setup)
	}

	this.setConfirm=function(confirm) {
		socket.emit(socketEvent(NETCODE_SETCONFIRM), confirm);
	}

	this.sendStartGame=function(gamesettings) {
		this.setState(NETPLAYSTATE_PLAYING);
		socket.emit(socketEvent(NETCODE_FREEZEROOM),gamesettings);
	}
	...
	this.sendEventTo=function (player,data) {
		if (this.state==NETPLAYSTATE_PLAYING)
			socket.emit(socketEvent(NETCODE_SENDEVENT),[player.netId,data]);
	}
	...
	this.broadcastPlayersData=function(data) {
		...
		if (outPacket[0].length||outPacket[1].length)
			socket.emit(socketEvent(NETCODE_BROADCAST),outPacket);
